...
//std::cout << ply << std::endl;
	...
	for (int i = 0; i < forced_moves.turns.size(); i++) {
		if (!board->chessmen[WHITE_][0].enabled) return (-_INFINITY_ + ply);
		if (!board->chessmen[BLACK_][1].enabled) return (_INFINITY_ - ply);
		forced_moves.pick(i);
...
//if (history.size() > 0)std::cout << ply << std::endl;
	...
	for (int i = 0; i < forced_moves.turns.size(); i++) {
		if (!board->chessmen[WHITE_][0].enabled) tmp = (-_INFINITY_ + ply);
		else if (!board->chessmen[BLACK_][1].enabled) tmp = (_INFINITY_ - ply);
		else {
		forced_moves.pick(i);
		forced_moves.turns[i]();
/*
...
*/
	...
	for (int i = 0; i < moves.turns.size(); i++) {
		if (!board->chessmen[WHITE_][0].enabled) tmp = (-_INFINITY_ + ply);
		else if (!board->chessmen[WHITE_][1].enabled) tmp = (_INFINITY_ - ply);
		else {
			moves.pick(i);
			moves.turns[i]();
/*
